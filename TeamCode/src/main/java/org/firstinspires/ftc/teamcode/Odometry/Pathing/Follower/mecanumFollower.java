...
public class mecanumFollower {
    ...
    public void followPath(double targetHeading, Odometry odometry, Drivetrain drive, Delivery delivery, Delivery_Slides deliverySlides) throws InterruptedException {
        ...
        do {
            ...
            if (Math.abs(robotPositionVector.getX() - targetPoint.getX()) < 1.4 && Math.abs(robotPositionVector.getY() - targetPoint.getY()) < 5 && Math.abs(odometry.getVerticalVelocity()) < 3 && Math.abs(odometry.getHorizontalVelocity()) < 3 && Math.abs(targetHeading - odometry.heading) < 2){
                reachedTarget = true;
            }
